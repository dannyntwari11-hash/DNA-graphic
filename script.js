...
if(menuToggle&&nav){menuToggle.addEventListener('click',()=>setMenu(!nav.classList.contains('active')));}
if(menuClose&&nav){menuClose.addEventListener('click',()=>setMenu(false));}
document.querySelectorAll('.main-nav a').forEach(link=>link.addEventListener('click',()=>setMenu(false)));
window.addEventListener('resize',()=>{if(window.innerWidth>800)setMenu(false);},{passive:true});
